...
export function UserTable({ users }: UserTableProps) {
  ...
  const overallUsers = users ? users.length : 0;
  const totalPages = Math.max(1, Math.ceil(overallUsers / itemsPerPage));
  const currentUsers = users ? users.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage) : [];
  
  const startCount = overallUsers > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0;
  const endCount = Math.min(currentPage * itemsPerPage, overallUsers);
