...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#1B2435] border-r border-slate-200 dark:border-slate-700/30 flex flex-col h-screen sticky top-0 transition-colors duration-200">
      
      <div className="h-24 flex items-center justify-between px-6 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/10 transition-colors">
        <div className="flex items-center gap-3">
          <div className="relative w-12 h-12 shrink-0 overflow-hidden rounded-xl shadow-sm border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-[#1E293B]">
          <img 
        ...
        <div className="flex flex-col justify-center">
          <h1 className="text-slate-900 dark:text-white font-bold tracking-wide text-lg leading-tight">ALERTO</h1>
      ...
      <nav className="flex-1 px-4 py-6 space-y-1.5">
        <Link 
          ...
          className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
            pathname === "/dashboard" 
              ? "bg-blue-50 dark:bg-slate-400/20 text-blue-600 dark:text-white" 
              : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-700/30" 
          }`}
        >
      ...
      <div className="p-6 mt-auto">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-700/50 flex items-center justify-center text-slate-700 dark:text-white font-bold">
            {initial}
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-semibold text-slate-900 dark:text-white">{displayName}</span>
            <span className="text-xs text-slate-500 dark:text-slate-500">{displayRole}</span>
          </div>
        </div>
        <button 
          onClick={async () => {
